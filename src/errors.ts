...
export function buildLaunchFailureMessage(args: {
  ...
  return [
    ...
    '',
    'A fresh throwaway profile was also tried and still failed, so this is not a',
    'stale-profile lock or a corrupted profile — the cause is environmental:',
    '',
    '  1. Antivirus blocking Chrome helper processes or the DevTools pipe',
    '     (Kaspersky, ESET, Norton, etc.). Add the Chromium folder to the AV',
    "     exclusions / trusted-applications list, then retry:",
    `       ${args.msPlaywrightDir}`,
    '',
    '  2. Chromium binary missing dependencies. Reinstall it forcibly:',
    '       npx playwright-core install chromium --force',
    '',
    '  3. As a last resort, move the profile aside so a fresh one is created:',
    `       ${args.profileDir}`,
    '',
